...
class voronoi_cell {
 public:
  ...
  source_index_type source_index() const { return source_index_; }
 ...
 private:
  ...
  enum Bits { BITS_SHIFT = 0x5, BITS_MASK = 0x1F };
...
class voronoi_vertex {
 public:
  ...
  voronoi_vertex(const coordinate_type& x, const coordinate_type& y)
      : x_(x), y_(y), incident_edge_(NULL), color_(0) {}
 ...
 public:
  ...
  voronoi_edge(bool is_linear, bool is_primary)
      : cell_(NULL),
        ...
        prev_(NULL),
        color_(0) {
    if (is_linear) color_ |= BIT_IS_LINEAR;
    if (is_primary) color_ |= BIT_IS_PRIMARY;
  }
  ...
  bool is_linear() const { return (color_ & BIT_IS_LINEAR) ? true : false; }

  // Returns true if the edge is curved (parabolic arc).
  // Returns false if the edge is linear (segment, ray, line).
  bool is_curved() const { return (color_ & BIT_IS_LINEAR) ? false : true; }

  // Returns false if edge goes through the endpoint of the segment.
  // Returns true else.
  bool is_primary() const { return (color_ & BIT_IS_PRIMARY) ? true : false; }

  // Returns true if edge goes through the endpoint of the segment.
  // Returns false else.
  bool is_secondary() const { return (color_ & BIT_IS_PRIMARY) ? false : true; }
...
class voronoi_diagram {
 public:
  ...
  const cell_container_type& cells() const { return cells_; }

  const vertex_container_type& vertices() const { return vertices_; }

  const edge_container_type& edges() const { return edges_; }
  ...
  std::pair<void*, void*> _insert_new_edge(
    ...
    if (cells_.empty()) {
      cells_.push_back(
          cell_type(site1.initial_index(), site1.source_category()));
    }

    // The second site represents a new site during site event
    // processing. Add a new cell to the cell records.
    cells_.push_back(cell_type(site2.initial_index(), site2.source_category()));
  ...
  void _build() {
    ...
    if (vertices_.empty()) {
      ...
      for (cell_iterator cell_it = cells_.begin(); cell_it != cells_.end();
           ++cell_it) {
        if (cell_it->is_degenerate()) continue;
        ...
        while (left_edge->prev() != NULL) {
          ...
          if (left_edge == cell_it->incident_edge()) break;
        }

        if (left_edge->prev() != NULL) continue;

        edge_type* right_edge = cell_it->incident_edge();
        while (right_edge->next() != NULL) right_edge = right_edge->next();
