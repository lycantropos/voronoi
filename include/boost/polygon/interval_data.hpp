...
class interval_data {
 public:
  ...
  coordinate_type low() const { return coords_[0]; }
  ...
  coordinate_type high() const { return coords_[1]; }
  ...
  bool operator<=(const interval_data& that) const { return !(that < *this); }

  bool operator>(const interval_data& that) const { return that < *this; }

  bool operator>=(const interval_data& that) const { return !((*this) < that); }
