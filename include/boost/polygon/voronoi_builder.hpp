...
template <typename T, typename CTT = detail::voronoi_ctype_traits<T>,
...
class voronoi_builder {
 public:
  ...
  std::size_t insert_segment(const int_type& x1, const int_type& y1,
 ...
 private:
  ...
  typedef typename VP::template event_comparison_predicate<site_event_type,
                                                           circle_event_type>
      event_comparison_predicate;
  typedef typename VP::template circle_formation_predicate<site_event_type,
                                                           circle_event_type>
  ...
  void init_beach_line_collinear_sites(OUTPUT* output) {
    ...
    while (it_second != site_event_iterator()) {
      ...
      beach_line_.insert(beach_line_.end(), std::pair<key_type, value_type>(
                                                new_node, value_type(edge)));
  ...
  void process_site_event(OUTPUT* output) {
    ...
    if (!site_event.is_segment()) {
      ...
      while (last != site_events_.end() && last->is_segment() &&
             last->point0() == site_event.point0())
        ++last;
    }
    ...
    for (; site_event_iterator() != last; ++site_event_index_) {
      ...
      if (right_it == beach_line_.end()) {
        ...
        activate_circle_event(left_it->first.left_site(),
                              left_it->first.right_site(), site_event,
                              right_it);
  ...
  void process_circle_event(OUTPUT* output) {
    ...
    it_first->second.edge(
        output
            ->_insert_new_edge(site1, site3, circle_event, bisector1, bisector2)
            .first);
  ...
  beach_line_iterator insert_new_arc(const site_event_type& site_arc1,
    ...
    position = beach_line_.insert(
        position, typename beach_line_type::value_type(
  ...
  void activate_circle_event(const site_event_type& site1,
    ...
    if (circle_formation_predicate_(site1, site2, site3, c_event)) {
      ...
      event_type& e =
          circle_events_.push(std::pair<circle_event_type, beach_line_iterator>(
