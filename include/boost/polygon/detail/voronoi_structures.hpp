...
class point_2d {
 public:
  ...
  point_2d(coordinate_type x, coordinate_type y) : x_(x), y_(y) {}
...
class site_event {
 public:
  ...
  site_event() : point0_(0, 0), point1_(0, 0), sorted_index_(0), flags_(0) {}

  site_event(coordinate_type x, coordinate_type y)
      : point0_(x, y), point1_(x, y), sorted_index_(0), flags_(0) {}

  explicit site_event(const point_type& point)
      : point0_(point), point1_(point), sorted_index_(0), flags_(0) {}

  site_event(coordinate_type x1, coordinate_type y1, coordinate_type x2,
             coordinate_type y2)
      : point0_(x1, y1), point1_(x2, y2), sorted_index_(0), flags_(0) {}

  site_event(const point_type& point1, const point_type& point2)
      : point0_(point1), point1_(point2), sorted_index_(0), flags_(0) {}

  bool operator==(const site_event& that) const {
    return (this->point0_ == that.point0_) && (this->point1_ == that.point1_);
  }
  ...
  coordinate_type x() const { return point0_.x(); }

  coordinate_type y() const { return point0_.y(); }

  coordinate_type x0() const { return point0_.x(); }

  coordinate_type y0() const { return point0_.y(); }

  coordinate_type x1() const { return point1_.x(); }

  coordinate_type y1() const { return point1_.y(); }

  const point_type& point0() const { return point0_; }

  const point_type& point1() const { return point1_; }

  std::size_t sorted_index() const { return sorted_index_; }
  ...
  std::size_t initial_index() const { return initial_index_; }
  ...
  bool is_inverse() const { return (flags_ & IS_INVERSE) ? true : false; }
...
class circle_event {
 public:
  ...
  coordinate_type lower_y() const { return center_y_; }
...
class ordered_queue {
 ...
 private:
  ...
  std::priority_queue<list_iterator_type, std::vector<list_iterator_type>,
                      comparison>
      c_;
  std::list<T> c_list_;
...
class beach_line_node_key {
 public:
  ...
  explicit beach_line_node_key(const site_type& new_site)
      : left_site_(new_site), right_site_(new_site) {}
  ...
  beach_line_node_key(const site_type& left_site, const site_type& right_site)
      : left_site_(left_site), right_site_(right_site) {}

  const site_type& left_site() const { return left_site_; }

  site_type& left_site() { return left_site_; }
  ...
  const site_type& right_site() const { return right_site_; }

  site_type& right_site() { return right_site_; }
...
class beach_line_node_data {
 public:
  explicit beach_line_node_data(Edge* new_edge)
      : circle_event_(NULL), edge_(new_edge) {}

  Circle* circle_event() const { return circle_event_; }
