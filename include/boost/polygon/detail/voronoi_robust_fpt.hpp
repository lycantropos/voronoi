...
class robust_fpt {
 public:
  ...
  enum { ROUNDING_ERROR = 1 };

  robust_fpt() : fpv_(0.0), re_(0.0) {}
  explicit robust_fpt(floating_point_type fpv) : fpv_(fpv), re_(0.0) {}
  ...
  bool has_pos_value() const { return is_pos(fpv_); }

  bool has_neg_value() const { return is_neg(fpv_); }

  bool has_zero_value() const { return is_zero(fpv_); }

  robust_fpt operator-() const { return robust_fpt(-fpv_, re_); }
  ...
  robust_fpt sqrt() const {
    return robust_fpt(
        get_sqrt(fpv_),
        re_ * static_cast<relative_error_type>(0.5) + ROUNDING_ERROR);
  }
...
class robust_dif {
 public:
  ...
  T dif() const { return positive_sum_ - negative_sum_; }

  T pos() const { return positive_sum_; }

  T neg() const { return negative_sum_; }
 ...
 private:
  void swap() { (std::swap)(positive_sum_, negative_sum_); }
...
class robust_sqrt_expr {
 public:
  ...
  _fpt eval2(_int* A, _int* B) {
    ...
    if ((!is_neg(a) && !is_neg(b)) || (!is_pos(a) && !is_pos(b))) return a + b;
