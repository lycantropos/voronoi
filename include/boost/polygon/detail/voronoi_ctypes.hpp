...
struct ulp_comparison<fpt64> {
  ...
  Result operator()(fpt64 a, fpt64 b, unsigned int maxUlps) const {
    ...
    if (ll_a < 0x8000000000000000ULL) ll_a = 0x8000000000000000ULL - ll_a;
    if (ll_b < 0x8000000000000000ULL) ll_b = 0x8000000000000000ULL - ll_b;
    ...
    if (ll_a > ll_b) return (ll_a - ll_b <= maxUlps) ? EQUAL : LESS;
...
struct extened_exponent_fpt_traits<fpt64> {
 public:
  ...
  enum { MAX_SIGNIFICANT_EXP_DIF = 54 };
};
...
class extended_exponent_fpt {
 public:
  ...
  bool is_pos() const { return val_ > 0; }

  bool is_neg() const { return val_ < 0; }
  ...
  extended_exponent_fpt operator+(const extended_exponent_fpt& that) const {
    if (this->val_ == 0.0 ||
        ((this->exp_ < std::numeric_limits<exp_type>::max() -
                           _traits::MAX_SIGNIFICANT_EXP_DIF) &&
         (that.exp_ > this->exp_ + _traits::MAX_SIGNIFICANT_EXP_DIF))) {
      return that;
    }
    if (that.val_ == 0.0 ||
        ((that.exp_ < std::numeric_limits<exp_type>::max() -
                          _traits::MAX_SIGNIFICANT_EXP_DIF) &&
  ...
  fpt_type d() const { return std::ldexp(val_, exp_); }
...
class extended_int {
 public:
  ...
  void neg() { this->count_ = -this->count_; }
  ...
  void mul(const extended_int& e1, const extended_int& e2) {
    ...
    if ((e1.count() > 0) ^ (e2.count() > 0)) this->count_ = -this->count_;
  }

  const uint32* chunks() const { return chunks_; }

  int32 count() const { return count_; }

  std::size_t size() const { return (std::abs)(count_); }
  ...
  std::pair<fpt64, int> p() const {
    ...
    if (this->count_ < 0) ret_val.first = -ret_val.first;
    return ret_val;
  }
 ...
 private:
  void add(const uint32* c1, std::size_t sz1, const uint32* c2,
           std::size_t sz2) {
    if (sz1 < sz2) {
      add(c2, sz2, c1, sz1);
      return;
  ...
  void dif(const uint32* c1, std::size_t sz1, const uint32* c2, std::size_t sz2,
           bool rec = false) {
    if (sz1 < sz2) {
    ...
    if (this->chunks_[this->count_]) ++this->count_;
  }

  void mul(const uint32* c1, std::size_t sz1, const uint32* c2,
           std::size_t sz2) {
