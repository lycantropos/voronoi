...
class point_data {
 public:
  ...
  coordinate_type x() const { return coords_[HORIZONTAL]; }
  ...
  coordinate_type y() const { return coords_[VERTICAL]; }
  ...
  bool operator!=(const point_data& that) const { return !(*this == that); }
  ...
  bool operator<=(const point_data& that) const { return !(that < *this); }
