...
class segment_data {
 public:
  ...
  point_type low() const { return points_[LOW]; }
  ...
  point_type high() const { return points_[HIGH]; }
  ...
  bool operator==(const segment_data& that) const {
    return (points_[0] == that.points_[0]) && (points_[1] == that.points_[1]);
  }
  ...
  bool operator<=(const segment_data& that) const { return !(that < *this); }

  bool operator>(const segment_data& that) const { return that < *this; }

  bool operator>=(const segment_data& that) const { return !((*this) < that); }
