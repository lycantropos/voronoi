...
void VoronoiDiagram::AddPoint(Point p) { points.push_back(p); }

void VoronoiDiagram::AddSegment(Segment s) { segments.push_back(s); }
...
std::vector<Point> VoronoiDiagram::GetPoints() { return points; }
...
c_Cell VoronoiDiagram::GetCell(long long index) const {
  ...
  if (cell->source_category() == boost::polygon::SOURCE_CATEGORY_SINGLE_POINT) {
    ...
    source_category = 2;
             ...
             boost::polygon::SOURCE_CATEGORY_INITIAL_SEGMENT) {
    source_category = 3;
  } else if (cell->source_category() ==
             boost::polygon::SOURCE_CATEGORY_REVERSE_SEGMENT) {
    source_category = 4;
  } else if (cell->source_category() ==
             boost::polygon::SOURCE_CATEGORY_GEOMETRY_SHIFT) {
    source_category = 5;
  } else if (cell->source_category() ==
             boost::polygon::SOURCE_CATEGORY_BITMASK) {
    source_category = 6;
  }
  ...
  c_Cell c_cell = c_Cell(index, cell->source_index(), cell->contains_point(),
                         cell->contains_segment(), is_open, source_category);
